...
def RESNET_V2(input_shape, num_classes=10):
    ...
    the number of filter maps is doubled. Within each stage, the layers
    have the same number filters and the same filter map sizes.
    Features maps sizes:
    conv1  : 32x32,  16
    stage 0: 32x32,  64
    stage 1: 16x16, 128
    ...
    outputs = Dense(num_classes,
                    activation='softmax',
                    kernel_initializer='he_normal', name='label')(y)

    # Instantiate model.
    model = keras.Model(inputs=inputs, outputs=outputs, name='RESNET-V2')
    return model
